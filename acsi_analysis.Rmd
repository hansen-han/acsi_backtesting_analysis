...
### What does satisfaction by year look like for each sector?
```{r}
for (input_year in score_data_long %>% pull(year) %>% unique) {
  print(score_data_long %>% filter(year == input_year) %>% ggplot(aes(color=sector, y = score, x=sector)) + geom_boxplot(alpha=0.4) + geom_jitter(size=0.4, alpha=0.9) + theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) + coord_flip() + theme(legend.position = "none") + ggtitle(paste0("Satisfaction by Sector in ", input_year)) + ylab("ACSI Score") + xlab(""))  +
  ylim(c(60, 100))
}
```
...
How many tickers do we have in total?
```{r}
...
Overall, it looks like the companies that were surveyed for inclusion in the ACSI had a cumulative higher performance than the S&P500. But since this can be skewed by a few good years (or reduced downturns, not overall more robust performance), I'm curious to see year to year changes....(TODO)
...
What about looking at specific years?
```{r}
...
```{r}
all_strategy_data %>% drop_na() %>% filter(year=="2021")
```
...
plot_cumulative_performance_test(all_strategy_data %>% drop_na())  + theme_bw()

```


```{r}
#why are we dropping the last point for this???
testdf <- all_strategy_data %>% drop_na() %>% filter(type=="99th Percentile Across All Sectors")
plot_cumulative_performance(testdf %>% drop_na()) + xlab("") + theme_bw() + theme(legend.position = "none", legend.title=element_blank())
```
...
x_labels <- c("1997 - 2001", "2002 - 2006", "2007 - 2011", "2012 - 2016", "2017 - 2021")  # Manually assign the x-axis labels
...
#+scale_color_viridis_d()  # Adjust the color scale if needed
```
